...
namespace BSLRMGWEB.Controllers
{
    public class ReportController : Controller
    {
        ...
        public ActionResult PilotReport()
        {
            return View();
        }
        ...
        public ActionResult QAQCDHUReport()
        {
            return View();
        }
        ...
        [HttpPost]
        public JsonResult Fn_Get_Piece_Rate_Report_Summary(clsPieceRateReportReq objReq)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri(Convert.ToString(ConfigurationManager.AppSettings["BSLRMGAPIURL"]));
                ...
                HttpContent content = new StringContent(DATA, UTF8Encoding.UTF8, "application/json");
                HttpResponseMessage responsePost = client.PostAsync("api/Report/Fn_Get_Piece_Rate_Report_Summary", content).Result;
                if (responsePost.IsSuccessStatusCode)
                {
                    return Json(new { success = true, message = responsePost.Content.ReadAsStringAsync().Result }, JsonRequestBehavior.AllowGet);
                }
        ...
        [HttpPost]
        public JsonResult Fn_Get_Pilot_Report(clsBundleStatusReportReq objReq)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri(Convert.ToString(ConfigurationManager.AppSettings["BSLRMGAPIURL"]));
                ...
                HttpContent content = new StringContent(DATA, UTF8Encoding.UTF8, "application/json");
                HttpResponseMessage responsePost = client.PostAsync("api/Report/Fn_Get_Pilot_Report", content).Result;
                if (responsePost.IsSuccessStatusCode)
                {
                    return Json(new { success = true, message = responsePost.Content.ReadAsStringAsync().Result }, JsonRequestBehavior.AllowGet);
                }
        ...
        #region Start Fn_Get_QAQCDHUReport 21-AUG-2026
        [HttpPost]
        public JsonResult Fn_Get_QAQCDHUReport(clsQADHUReport objReq)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri(Convert.ToString(ConfigurationManager.AppSettings["BSLRMGAPIURL"]));
                ...
                HttpContent content = new StringContent(DATA, UTF8Encoding.UTF8, "application/json");
                HttpResponseMessage responsePost = client.PostAsync("api/Report/Fn_Get_QAQCDHUReport", content).Result;
                if (responsePost.IsSuccessStatusCode)
                {
                    return Json(new { success = true, message = responsePost.Content.ReadAsByteArrayAsync().Result }, JsonRequestBehavior.AllowGet);
                }
                else
                {
                    return Json(new { success = false, message = "QA DHU Report records are not getting." }, JsonRequestBehavior.AllowGet);
                }
            }
        }
        #endregion End Fn_Get_QAQCDHUReport 21-AUG-2026

        #region Start Fn_Get_Rate_OpNo 03-SEP-2026 Added by Ankit

        [HttpPost]
        public JsonResult Fn_Get_Rate_By_OpNo(clsOPBreackDownMaster objReq)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri(Convert.ToString(ConfigurationManager.AppSettings["BSLRMGAPIURL"]));
                ...
                HttpContent content = new StringContent(DATA, UTF8Encoding.UTF8, "application/json");
                HttpResponseMessage responsePost = client.PostAsync("api/Report/Fn_Get_Rate_By_OpNo", content).Result;
                if (responsePost.IsSuccessStatusCode)
                {
                    return Json(new { success = true, message = responsePost.Content.ReadAsStringAsync().Result }, JsonRequestBehavior.AllowGet);
                }
                ...
                {
                    return Json(new { success = false, message = "Rate_OpNo failed." }, JsonRequestBehavior.AllowGet);
                }
            }
        }
