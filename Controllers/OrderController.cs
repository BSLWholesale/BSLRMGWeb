...
namespace BSLRMGWEB.Controllers
{
    public class OrderController : Controller
    {
        ...
        [HttpPost]
        public JsonResult Fn_Get_Fabric_ColorLits(clsFabricColor objReq)
        {
            using (var client = new HttpClient())
            {
                client.BaseAddress = new Uri(ConfigurationManager.AppSettings["BSLRMGAPIURL"]);
                ...
                string DATA = Newtonsoft.Json.JsonConvert.SerializeObject(objReq);
                HttpContent content = new StringContent(DATA, UTF8Encoding.UTF8, "application/json");
                HttpResponseMessage responsePost = client.PostAsync("api/Order/Fn_Get_Fabric_ColorLits", content).Result;
        ...
        [HttpPost]
        public JsonResult Fn_Update_OpNo_IN_OBD(clsOPBreackDownDetail objReq)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                HttpResponseMessage responsePost = client.PostAsync("api/Order/Fn_Update_OpNo_IN_OBD", content).Result;
                if (responsePost.IsSuccessStatusCode)
                {
                    return Json(new { success = true, message = responsePost.Content.ReadAsStringAsync().Result }, JsonRequestBehavior.AllowGet);
                }
